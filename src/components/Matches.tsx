...
export function Matches(props: PropsClass<MatchesProps>) {
  const today = () => dayjs()
  const matchesOrdered = createMemo(() => getMatchesOrdered(props.matches))
  const closestIndex = createMemo(() => getClosestDay(matchesOrdered(), today().toDate()))
  ...
  return <div class={twMerge("p-14 flex flex-col gap-2", props.class)}>
    <For each={matchesOrdered()} fallback={<div>No matches</div>}>
      {(matchOrdered, index) => {
        ...
        return <div class={`w-full ${props.mode !== 'homepage' && isToday() ? "bg-custom-yellow-100 bg-opacity-10 p-2 rounded-lg" : ""} `} >
          {props.mode !== 'homepage' && <span class={twMerge(
            ...
            <div class="flex justify-between">
              ...
              <Show when={isClosest()}>
                <div class="flex gap-2 text-custom-yellow-100 items-center">
