...
function TeamWinrateElement({
  opponent,
  avgLength,
  nbMatches,
  winrate
}: TeamWinrateElementProps) {
  return <div class={"flex justify-between gap-6 items-center text-custom-white-200 whitespace-nowrap"}>
    <A href={`/team/${opponent.name}`} class="flex items-center gap-2 text-sm font-semibold text-custom-yellow-100">
      <img alt={opponent.acronym} src={opponent.image_url} width={32} height={32} />
      {opponent.acronym}
    </A>
    <div class="flex flex-col gap-px items-center">
      <span>
        <span class="font-semibold">{(avgLength / 60).toFixed(0)}</span> min
      </span>
      <span class="text-xs">
        average
      </span>
    ...
    <div class="flex flex-col gap-px items-center">
      <span>
        <span class="font-semibold">{winrate.toFixed(0)}</span>%
      </span>
      <span class="text-xs">
        {nbMatches} match
      </span>
