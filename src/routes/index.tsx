...
import { logoutAction$, useSession } from "~/lib/server";
...
export function routeData() {
  ...
  return { usersData, matchesOfTheDay, upcomingMatches, user, bets }
}


export default function Home() {
  const { usersData, matchesOfTheDay, upcomingMatches, user, bets } = useRouteData<typeof routeData>()
  ...
  return <main class="flex h-full w-full p-6 gap-6">
    <div class="w-full flex flex-col gap-6 h-full">
      ...
      <div class="flex gap-6 h-1/2">
        <SectionWithTitle title={"Profil"} class="min-w-max max-w-xs">
